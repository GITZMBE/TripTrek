...
export const CategoryButton = ({ icon: Icon, label, selected, ...props }: CategoryButtonProps) => {
  return (
    <button className={`w-2/5 p-4 rounded-lg hover:text-light border-2 hover:border-light space-y-2 ${ selected ? 'text-light border-2 border-light' : 'text-grey border-grey' }`} {...props}>
      <Icon size={24} />
